...
export function initToolbar(editor) {
  ...
  for (let group in tools) {
    ...
    for (let tool in tools[group]) {
      ...
      let hotkey = opts.hotkeys ? (isIOS() ? opts.hotkeys.ios : opts.hotkeys.win) : '';

      toolsString += `<button type="button" data-command="${group}" data-option="${tool}" data-title="${opts.title}" data-hotkey="${hotkey}">
